...
// that *start* with a keyword (e.g. INVOICE, RETURNS_DATA) are not split.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Function-level keyword tokens
//
// Prefix-conflict ordering rules (longest declared first within group):
//   INVOKER  before  IN   (both start with "IN")
//   INTEGER  before  IN   (INTEGER starts with "IN")
//
// Note: OUT and INOUT are intentionally absent — HANA functions accept only
// IN parameters, so these tokens serve no purpose and their absence prevents
// any OUT/INOUT-shaped body SQL from being misinterpreted as parameter modes.
// ---------------------------------------------------------------------------
...
export const FunctionKw = createToken({ name: 'Function', pattern: /FUNCTION/i, longer_alt: Identifier });
...
//   NCLOB     before CLOB  (by convention; no shared prefix conflict)
// ---------------------------------------------------------------------------
...
//     - Invoker before In  (IN-prefix group)
//     - Integer before In  (INTEGER starts with IN)
...
export const allTokens: TokenType[] = [
    ...
    StringLiteral,
    // Function keywords — IN-prefix group: longest first.
 ...
 */
export const HdbFunctionLexer = new Lexer(allTokens);
